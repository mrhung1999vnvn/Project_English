...
export default function Example({navigation,route}) {
    ...
    const dataRoute = route.params?route.params.data[route.params.data.length-1]:dContext.dictionary.data
    /**
    ...
    return (
        <Container>
            {exitStates && (
                <View style={{  position:'absolute',justifyContent:'center',alignItems:'center',top:0,left:0,right:0,bottom:0,backgroundColor:'rgba(0,0,0,.6)',zIndex:999}}>
                    <View style={{ padding:10,borderRadius:10,backgroundColor:'#fff',width:'90%',elevation:2,justifyContent:'center',alignItems:'center'}}>
                        <View style={{backgroundColor:'#0A4267',width:'40%',borderRadius:30,overflow:'hidden',marginBottom:20}}>
                            <Image  style={{width:'100%',alignSelf:'center',height:100,borderRadius:90}} source={require('../../../assets/image/look.gif')}  resizeMethod='resize' resizeMode='stretch' />
                        </View>
                        <TextRN style={{fontWeight:'bold',marginBottom:20, color:'#0A4267',fontSize:25}}>It's so sad, good bye?</TextRN>

                        <View style={{width:'100%',borderRadius:10, backgroundColor:'rgba(255,255,255,.7)', flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                            <ButtonFlat onPress={_close} style={{flex:1,justifyContent:'center',backgroundColor:'gray'}}>Exit</ButtonFlat>
                            <ButtonFlat onPress={_animHide} style={{flex:1,justifyContent:'center',backgroundColor:'#0A4267'}}>Continue</ButtonFlat>
                        </View>
                    </View>
                </View>
            )}
            ...
            <FlatList
                ...
                data={dataRoute.word}
                renderItem={({item,index}) => <Item answer={DICTIONARY_1[`${item}`].answer} onPress={()=>_onPress()} questions={DICTIONARY_1[`${item}`].question} index={index} title={item} />}
